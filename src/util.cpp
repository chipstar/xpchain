...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No xpchain.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override xpchain.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", XPCHAIN_PID_FILENAME));
