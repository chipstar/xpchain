...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete XPChain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no XPChain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old XPChain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected XPChain 0.7 datadir."
    ;;
  3)
    echo "Detected XPChain pre-0.8 datadir."
    ;;
  4)
    echo "Detected XPChain 0.8 datadir."
    ;;
esac
