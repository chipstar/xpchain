...
%description
XPChain is a digital cryptographic currency that uses peer-to-peer technology to
...
to run a XPChain wallet, this is probably the package you want.
%endif


%package libs
Summary:	XPChain shared libraries
...
Encoding=UTF-8
Name=XPChain
Comment=XPChain P2P Cryptocurrency
Comment[fr]=XPChain, monnaie virtuelle cryptographique pair à pair
Comment[tr]=XPChain, eşten eşe kriptografik sanal para birimi
Exec=bitcoin-qt %u
Terminal=false
Type=Application
Icon=bitcoin128
...
getent passwd bitcoin >/dev/null ||
	...
	-c "XPChain wallet server" bitcoin
exit 0
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8762
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8798
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18762
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18798
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 28762
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 28798
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8762
	%{_sbindir}/semanage port -d -p tcp 8798
	%{_sbindir}/semanage port -d -p tcp 18762
	%{_sbindir}/semanage port -d -p tcp 18798
	%{_sbindir}/semanage port -d -p tcp 28762
	%{_sbindir}/semanage port -d -p tcp 28798
	for selinuxvariant in %{selinux_variants}; do
