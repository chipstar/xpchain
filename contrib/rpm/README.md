...
The RPM spec file provided here is for XPChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://xpchain.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[XPChain git repository](https://github.com/xpchain/xpchain) but are not part of
...
in the bitcoin git repository. At present only an SVG version of the XPChain
icon is used.
...
needed when 0.12.1 is released, a proper fix is already in the XPChain git
master branch.
...
idea to build XPChain Core against a static version of that library compiled
...
XPChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
packaging XPChain for Fedora at least since 2012.
